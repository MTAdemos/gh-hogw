...
jQuery.fn.extend({
  new_pwrap: function(){
    ...
    $(this).parent().parent().next().HandleCanNotDelete(1);
  }
});
...
jQuery.fn.extend({
  new_reissue: function(){
    $(this).parent().next().after(discogreissuestr1 + "\n" + discogreissuestr2);
  }
});
...
discogreissuesecondarystr+="<div class=inner contenteditable=false onclick='$(this).new_reissue();'>add re-issue </div> ";
discogreissuesecondarystr+="</div>\n";

// add marker for re-issue after last re-issue, or after release if no re-issue. The insertion
//  point is after the <p class=info> if there is one

      var AddReissuePoint="";
      var reissuedlen=$(this).find(".reissued").length;
      if(reissuedlen>0){
        AddReissuePoint=$(this).find(".reissued").eq(reissuedlen-1);
        if(AddReissuePoint.next().find(".info").length>0)AddReissuePoint=AddReissuePoint.next();
        }
      else{
        var releasedlen=$(this).find(".released").length;
        if(releasedlen>0){
          AddReissuePoint=$(this).find(".released").eq(releasedlen-1);
          if(AddReissuePoint.next().find(".info").length>0)AddReissuePoint=AddReissuePoint.next();
        }
      }
      if(AddReissuePoint){
        AddReissuePoint.before(discogreissuesecondarystr);
        }
      else{
        alert("Can't find the place to add a reissue. Please contact Mark");
      }
