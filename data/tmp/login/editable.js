...
function addYoxviewImage(ID){
  ...
  if(CanContinue>0){
     ...
     if(typeof workimgheight==="undefined"){workimgheight="*";}else{if(workimgheight==0)workimgheight="*";}
     if(typeof workimgwidth==="undefined"){workimgwidth="*";}else{if(workimgwidth==0)workimgwidth="*";}
...
discogreissuestr2+="  <p class='info CanNotDelete' contenteditable=true>\n";
...
jQuery.fn.extend({
  new_reissue: function(){
    // this gets called  after various possible entries, e.g. Released, but needs
    // to skip on one if called after "reisssued" to allow for an info section
    // when it is added after a re-issed entry it has a CanNotDelete wrapper, so needs to go up one level

    var AdditionPoint=$(this).parents(".reissuesecondary");
    if(AdditionPoint.parent().hasClass("CanNotDeleteWrapper"))AdditionPoint=AdditionPoint.parent();
    AdditionPoint=AdditionPoint.next();

    if(AdditionPoint.hasClass("reissued"))AdditionPoint=AdditionPoint.next();
...
notestr+="    <div class='tag CanNotDelete'>Note tag</div><div class='notecontent CanNotDelete'>Detail</div>";
notestr+="  </div>";
...
// publications
var reviewsecondarystr="";
reviewsecondarystr+="<div class='reviewsecondary spaceholder'>";
reviewsecondarystr+="<div class=inner contenteditable=false onclick='$(this).new_review();'>new review</div>";
reviewsecondarystr+="</div>";

var reviewstr="   <div class=reviewquote>quote from review</div>\n";
...
jQuery.fn.extend({
  new_review: function(){
    ...
    $(this).parents(".reviewsecondary").replaceWith(reviewstr);
  }
});
  ...
  edit: function(){
    ...
    if($(".publication").length>0){
      $(".publication").each(function(){
        $(this).find(".reviewquote").each(function(){
          $(this).after(reviewsecondarystr);
        });
        if($(this).find(".reviewquote").length<1){
          $(this).find(".textwrapper").append(reviewsecondarystr);
        }
      });
    }
...
jQuery.fn.extend({
  save: function(pubflag){
    ...
    if(location.pathname.indexOf("feedback.htm")>-1){
      $(".OKToPublishbox").remove();
    }
    ...
    if(location.pathname.indexOf("feedback.htm")>-1){
      ...
      var div_for_published_content=$(div_to_write).clone();
//alert("before: " + div_for_published_content.not(".OKToPublish").length);
//alert("div_for_published_content: " + div_for_published_content.html());

      div_for_published_content.find(".feedbackentry").each(function(){
        if($(this).hasClass("OKToPublish")){}else{$(this).remove();}
      });

//alert("after: " + div_for_published_content.not(".OKToPublish").length);
//alert("div_for_published_content: " + div_for_published_content.html());

      WriteToFile($(div_to_write).html(),file_to_write,0,1);
      if(pubflag>0){
        WriteToFile(div_for_published_content.html(),file_to_write,1,1);
      }
...
//// NB save to /tmp first (pubflag>0) as feedback items without class OKToPublish are removed before saving
//    if(pubflag>0){
//alert("file_to_write: " + file_to_write);
//      WriteToFile($(div_to_write).html(),file_to_write,1,1);
//      if(location.pathname.indexOf("feedback.htm")>-1){
//        $(".editable").each(function(){
//          if($(this).hasClass("OKToPublish")){
//            }
//          else{
//            alert("removing: " + $(this).html());
//            $(this).remove();
//          }
//        });
//      }
